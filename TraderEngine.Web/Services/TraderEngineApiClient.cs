...
public class TraderEngineApiClient : ITraderEngineApiClient
{
  ...
  private async Task<HttpResponseMessage> PostAuthenticated<TBody>(
    AppUser user, string requestUri, TBody body, Guid? actingAsClientId, CancellationToken ct)
  {
    var (token, _) = _jwtTokenService.GenerateToken(user, actingAsClientId);
