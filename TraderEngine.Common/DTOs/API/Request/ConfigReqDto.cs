...
public class ConfigReqDto
{
  ...
  [Display(Name = "Max asset count [n]",
    Description =
      "Max amount of assets from CoinMarketCap listing, ranked by market cap. Excluding quote, and assets that contain ignored tags, i.e. they are not counted.")]
  [Range(0, 100)]
  public int TopRankingCount { get; set; } = 10;

  [Display(Name = "EMA smoothing [hrs]",
    Description = "Exponential Moving Average period of Market Cap, to smooth out volatility.")]
  [Range(1, 72)]
  public int Smoothing { get; set; } = 8;

  [Display(Name = "Nth root ^(1/[n])",
    Description = "Nth root of Market Cap EMA, to dampen the effect an individual asset has on the portfolio.")]
  [Range(1, 25)]
  public double NthRoot { get; set; } = 2.5;

  [Display(Name = "Min order size",
    Description = "The minimum order size in quote required to trigger an automated rebalance.")]
  ...
  [Display(Name = "Enable automation",
    Description = "Automatically perform portfolio rebalance when conditions are met.")]
  public bool AutomationEnabled { get; set; } = false;

  [Display(Name = "Use limit orders",
    Description =
      "Place limit orders at the best bid/ask to reduce fees, falling back to a market order for any unfilled remainder. Note that it may take longer to fill limit orders than market orders.")]
  ...
  [Display(Name = "Held-asset bias mult",
    Description =
      "Selection-ranking multiplier applied to currently-held assets. Values above 1 bias toward retaining current holdings, reducing churn on borderline-ranked assets without distorting their computed weights.")]
